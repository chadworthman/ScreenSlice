...
MenuBar (start) → AppDelegate orchestrates →
  ...
  2. VirtualDisplayManager creates CGVirtualDisplay (1920×1080)
...
| `MenuBarController.swift` | NSStatusItem menu with start/stop, aspect ratio presets, audio toggle |
...
| `OverlayContentView.swift` | Even-odd fill dimming, 8 resize handles, drag bar, aspect ratio enforcement |
...
- **Callback-based wiring**: Managers communicate via closures (`onFrameReceived`, `onRegionChanged`, `onStartStop`) set up in `AppDelegate.applicationDidFinishLaunching`. No delegates or Combine.
...
- Virtual display is always 1920×1080 regardless of capture region size
